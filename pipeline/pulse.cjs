...
function flagOf(r, farmActors = null) {
  ...
  //  · high-volume zero-human churner (≥25 pushes, ≤2 actors) even with a
  //    real-looking name — the ppfdalerts-style accounts.
  if (autoGen && zeroHuman && pushes >= 5 && actors <= 2) return 'push-bot';
  if (zeroHuman && pushes >= 25 && actors <= 2) return 'push-bot';
  ...
  // KNOWN FARM ACTOR: repos rotate, actors persist. Once an account has been
  // confidently flagged (auto-gen repo) in ≥2 distinct hours, any NEW repo it
  // pushes to is suspect — but keep it in the light 'push-loop' tier unless
  // the repo name is itself auto-generated, so we never call a possibly-real
  // project a "bot" on name-actor evidence alone.
  if (farmActors && r.actor_names && r.actor_names.some((a) => {
    const e = farmActors.get(a);
...
// farmActors: Map<actor, {lastSeen, hours:Set<hourLabel>}> persisted in
// site/data/farm_actors.json. An actor enters the ledger when its repo is
// confidently flagged push-bot via an auto-generated name; it only DEMOTES
// other repos once seen flagged in ≥2 distinct hours (repeat offender), and
// stale entries (not seen for 48h) are pruned so the ledger can't grow
// unbounded. Repos rotate, actors persist — this is the durable signal.
...
function buildSnapshot(hourLabel, agg) {
  ...
  // Farm-actor ledger: record actors behind *auto-generated-name* farms so
  // future runs demote any repo they touch (repos rotate, actors persist).
  // Only auto-gen names seed the ledger — a real solo dev who trips the
  // zero-human rule once (e.g. importing a big repo) must NOT be blacklisted
  // forever; the profile rules re-catch them per-hour anyway. An actor only
  // demotes OTHER repos once seen in ≥2 distinct hours.
  for (const b of allFlagged) {
    if (isAutoGeneratedName(b.repo)) {
      for (const a of b.actor_names || []) {
  ...
  const demoted = [...allFlagged, ...allSuspicious]
    ...
    .slice(0, 15)
    .map((r) => ({ repo: r.repo, flag: flagOf(r, FARM_ACTORS), pushes: r.pushes, actors: r.actors, human: (r.prs || 0) + (r.issues || 0) + (r.stars || 0) + (r.forks || 0) + (r.releases || 0) + (r.reviews || 0) }));
  return {
...
async function main() {
  ...
  try {
    ...
    if (obj && typeof obj === 'object') {
      FARM_ACTORS = new Map(Object.entries(obj).map(([a, e]) => [a, { hours: new Set(e.hours || []), lastSeen: e.lastSeen || latest }]));
