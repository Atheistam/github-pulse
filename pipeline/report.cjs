...
const timeline = [
  ...
  { when: "Aug 16 · hours 10–12", title: "The noon ritual — a farm that punches the clock (v5.26)", body: "Sixth consecutive lead-time live test PASSED: the h9 minting (608) fed a sustained breach — h10 62.7% → h11 51.7% → h12 55.4%, four straight ≥50% hours (h9–h12), wave radar PEAK at h12. But the headline is a NEW BEHAVIORAL FINGERPRINT, the strongest in 133 hours: loganfoxdale/d3 has appeared at EXACTLY hour 12 for SIX consecutive days — 08-11-12 312/hr, 08-12-12 351, 08-13-12 183, 08-14-12 227, 08-15-12 119, 08-16-12 109 — same actor, same repo name d3, a noon-UTC ritual so regular it looks cron-scheduled. The eternal #1 finally slipped: ugmoddev fell OUT of the top-15 farm list for the first time in 133 hours — it pushed 70/hr at h12 (still the 4th most-active actor on GitHub, 128 events) but the top-15 cutoff was 71: rate 158 → 158 → 70, structural decay from the 364/hr peak. elad-cmd returned at h10 (75/hr) after a 4h off — then off again h11–h12, the shift pattern intact. Janfindl's 3-on/1-off cadence BROKE: on h6–h8 (123/77/89), then FOUR straight off-hours (h9–h12) — a morning-shift pattern (active 06:00–08:00 UTC both days). zerotraceh1's re-ramp was a fade: weakest-ever 68/60 then gone. The hyphen+digit squad (miller-e6513, smithm87019) is NOT one-hit — it runs solo on/off shifts (83/0/64/0). NEW DISGUISE TIER: real-word usernames — forgetpwd, twainswee, spl1ce, janbeoty, stormart01, reinatomait — the generator now emits actual English words. Demotion record spree: h9 3,585 (record) + h10 3,225 (2nd-biggest ever) + h11 2,900 (6th) — three of the top-6 all-time demotion hours in ONE window. The factory's magazine is spent: 5,626 minted h5–h9, now re-arming at 214/449/440. 63 of 133 hours ≥50% · 21.73M events." },
];
...
<main class="wrap">
  ...
  <section class="panel">
    <div class="panel-head"><h2><span class="h-num">03</span> The 3-hour cadence &amp; the account factory</h2><span class="panel-sub">v5.27 — THE PLATEAU HOLDS: 7 straight ≥50% hours (h9-h15, 2nd-longest streak in 136h), PEAK did not collapse; noon ritual day six (109/hr at exactly h12) but decaying −69% from peak; ugmoddev out of top-15 all window; demotion record books rewritten (h14 3,277 #2 + h15 3,250 #3 all-time)</span></div>
    ...
    ${hoursSincePeak !== null ? `<div class=\"lede\" style=\"margin-top:16px;border-left:3px solid #fbbf24;padding-left:12px\"><b>VERDICT (v5.27): THE PEAK BECAME A PLATEAU, AGAIN — AND THE NOON RITUAL ENTERS DAY SIX, DECAYING.</b> Seven straight ≥50% hours (h9–h15: 64.1 → 62.7 → 51.7 → 55.4 → 53.4 → 59.0 → 58.2) — the second-longest breach streak in 136h, behind only the 10-hour siege; the PEAK did not collapse, the h11 dip (51.7%) recovered without a crash, post-peak-collapse falsified a third time. The noon ritual: <b>loganfoxdale/d3 pushed 109/hr at EXACTLY hour 12 and ZERO pushes at h11, h13, h14, h15</b> — the h12-only fingerprint survived a full window of negative checks, day six — but the ritual is decaying (312 → 351 → 183 → 227 → 119 → 109, <b>−69% from peak</b>). ugmoddev was OUT of the top-15 farms the ENTIRE window (84/0/62/hr) — the eternal #1's fade is structural; it survives only as the #1 persistent botnet (API-NEW-NAT-3-, 11 of last 13h). <b>The demotion record books got rewritten: h14 3,277 (#2 all-time) + h15 3,250 (#3)</b> — SIX of the top-12 all-time demotion hours now sit in the last 7 hours (h9–h15); enforcement is running hotter than the minting. The factory never stopped (515/409/579 — steady ~500/hr, no single magazine). Rotation: elad-cmd ran ONE shift at h15 (70/hr, #1 farm) after 4h off; Janfindl off all window (morning shift h6–h8 is its confirmed pattern — botnet watch still lists AFL_Live, seen 3h); the real-word disguise tier persisted (twainswee, janbeoty — 5h each, mid-duration shift workers, NOT one-hits); spl1ce now spans 10h over 3 days. Naming templates rotated hourly: name+digits squad (h13, all ~92–101/hr synchronized) → mixed/gibberish (h14, 71–75 cluster) → veteran + template (h15, 64–70 cluster). Ledger 29,153 actors (26,845 confirmed). <b>66 of 136 hours ≥50% · 22.24M events.</b></div>` : ''}
  </section>
