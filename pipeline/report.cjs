...
<main class="wrap">
  ...
  <section class="panel">
    <div class="panel-head"><h2><span class="h-num">03</span> The 3-hour cadence &amp; the account factory</h2><span class="panel-sub">v5.11 — rule confirmed: the ≥50% breach landed at h8 (55.6%), predicted from minting ~6h out</span></div>
    ...
    ${hoursSincePeak !== null ? `<div class="lede" style="margin-top:16px;border-left:3px solid #10b981;padding-left:12px"><b>Verdict (v5.11): rule CONFIRMED — the ≥50% breach landed, predicted ~6h out.</b> The run-23 prediction (minting 629 → 1,175 fresh accounts at h2/h3 → ≥50% surge) is now fully closed out: h7 <b>43.7%</b> (one pause hour), then h8 <b>55.6%</b> — the formal breach, first ≥50% hour in 17 — and h9 <b>51.2%</b>. Timing: the outer edge of the window +1 (h8, ~6h after the first ≥500 mint hour). The rotation is total: by h9 neither LiamBruhin nor ugmoddev charts at all — this surge is 100% fresh batch (sweeneyrachel2528 234, trujillojoshua139 230, thompsoncarrie2241 228, taylorlisa8 226, suttonveronica8 223, thomasmary5697 222 pushes/hr), and minting has already decelerated (h7 87 → h8 210 → h9 188 fresh accounts): the batch is deployed, the factory is re-arming. The mint-and-burn model is confirmed end-to-end: <b>mint at ≥500/hr → deploy the batch 2–6h later → ≥50% surge</b>. Naming template flipped back to firstname+lastname+digits (template A) for h7–h9 — same as the h4–h6 batch. ${vDemoted6} farm repos demoted in h6, and h7–h9 demoted 1,524 / 1,766 / 1,597 more.</div>` : ''}
  </section>
