// run49_analysis.cjs — h16-h19: POST-INCIDENT REGIME (GitHub critical incident 13:40Z->?)
// Questions from run 48's plan:
//  (a) POST-INCIDENT REGIME: wave re-breach >=50% now that GitHub is stable?
//      (h16 standing army re-engaged: ugmoddev 307 / elad-cmd 218 / zerotraceh1 164 + 219 minted
//       — siege resuming or one-shot re-engagement?)
//  (b) INCIDENT AFTERMATH: h16-h19 event volume back to ~150K/hr norm or still depressed?
//      (status.github.com still Partial System Outage at 16:59Z — resolved by now?)
//  (c) bogdanstancu 801/hr all-time high: hold/decay? new spray farms in 1-farm regime?
//  (d) factory: 219-mint at h16 -> magazine (>=500/hr) or one-shot?
//  (e) loganfoxdale: day-8 noon window is TOMORROW h12; negative window today = h17-h19
//  (f) zerotraceh1: 3rd comeback stable or one-night?
//  (g) elad-cmd: shift cadence after psycho-usage debut (218/hr)
//  (h) ledger trajectory (5th straight decline?)
//  (i) HN: 49310247 alive 55h+? karma still 3 -> no posts
//  (j) report v5.35
const fs = require('fs');
...
// ---- (a) TRAJECTORY h0 -> END ----
console.log('\n=== (a) SPAM TRAJECTORY h0 -> END (POST-INCIDENT REGIME: re-breach or continued collapse?) ===');
let prev = null;
...
// ---- (b) MINTING ----
console.log('\n=== (b) MINTING h10 -> END (factory after 219 at h16: magazine or one-shot?) ===');
console.log(series.filter(x => cmp(x.hour, '2026-08-17-10') >= 0).map(k => `${k.hour.slice(11)}:${batch[k.hour] || 0}(c${conf[k.hour] || 0})`).join('  '));

// ---- (b2) EVENTS RECOVERY: h16-h19 vs 7-day norm (incident aftermath) ----
console.log('\n=== (b2) EVENTS RECOVERY: h16-h19 vs 7-day same-hour norm (incident aftermath) ===');
const days7 = series.filter(s => cmp(s.hour, '2026-08-10-0') >= 0 && cmp(s.hour, '2026-08-17-16') < 0);
...
console.log('\n=== (b3) FARM PUSH VOLUME (sum bot_watch pushes/hr, last 15h) ===');
for (const s of series.slice(-15)) {
...
// ---- (c) LOGANFOXDALE noon ritual: negative window h17-h19 ----
...
// negative-window check h17-h19 (today's post-noon negative window)
const negWin = {};
for (const s of series) {
  if (!/2026-08-17-1[789]$/.test(s.hour)) continue;
  negWin[s.hour] = 0;
  for (const f of (s.bot_watch || [])) for (const a of (f.actor_names || [])) if (a === 'loganfoxdale') negWin[s.hour] += f.pushes;
}
console.log('loganfoxdale h17-h19 (negative window):', Object.keys(negWin).sort(cmp).map(h => `${h.slice(11)}:${negWin[h]}`).join('  '));

// ---- (c2) H12-ONLY COHORT recheck with h17-h19 now included (leak test) ----
console.log('\n=== (c2) H12-ONLY COHORT Aug17 (recheck w/ h17-h19: did the noon cohort leak?) ===');
const h12only = {};
for (const s of series) {
...
// ---- (d) SPRAY FARMS h17-h19 ----
console.log('\n=== (d) SPRAY FARMS last 16h (bogdanstancu 801 all-time high: hold/decay?) ===');
for (const who of ['bogdanstancu1119-maker', 'BailiffDisengage', 'hayesjonathan9829', 'DevZonayed', 'animal-lover12']) {
  ...
  const k = Object.keys(t).sort(cmp);
  console.log(`${who.padEnd(22)} ${k.slice(-16).map(h => `${h.slice(11)}:${t[h]}`).join(' ')}`);
}

// ---- (d) TOP-12 ACTORS h17-h19 ----
console.log('\n=== (d) TOP-12 ACTORS h17-h19 ===');
for (const s of series.slice(-3)) {
  const ta = (s.top_actors || []).slice(0, 12).map(a => `${a.actor}(${a.events})`).join('  ');
  console.log(`${s.hour}  ${ta}`);
}
console.log('\n=== TOP FARMS h17-h19 (bot_watch top-6) ===');
for (const s of series.slice(-3)) {
...
// ---- (e) ZEROTRACEH1 ----
console.log('\n=== (e) zerotraceh1 FULL TRACE (last 120h, 3rd comeback watch) ===');
...
// ---- NEW ACTORS ----
console.log('\n=== NEW ACTORS first_seen h16-h19 ===');
const namesByHour = {};
for (const [name, e] of Object.entries(ledger)) {
  const hrs = (e.hours || []).slice().sort(cmp);
  const f = hrs[0];
  if (f && cmp(f, '2026-08-17-16') >= 0) (namesByHour[f] = namesByHour[f] || []).push(name);
}
...
console.log('h17-h19 demotions:', series.slice(-3).map(k => `${k.hour.slice(11)}:${k.demoted_total}`).join('  '));

// ---- LEDGER ----
console.log('\n=== LEDGER ===');
const nLed = Object.keys(ledger).length;
const nConf = Object.values(ledger).filter(e => e.confirmed).length;
console.log(`ledger entries: ${nLed} (confirmed ${nConf})  [run48: 24,471 / 22,650]`);
