// run27_analysis.cjs — THE BIG TEST: record 2,002 mint at h12 -> does the surge land in h13-h16?
// Questions:
//  (a) spam trajectory h13-h15: surge landed? where? biggest jump ever?
//  (b) minting h13-h15: factory kept re-arming after the 2,002 record, or exhausted?
//  (c) h12 batch (loganfoxdale, williamsjacob1) carrying the surge, or another rotation?
//  (d) old guards LiamBruhin/ugmoddev: status
//  (e) naming template cadence h13-h15
const fs = require('fs');
...
for (const f of files) {
  series.push(JSON.parse(fs.readFileSync(path.join(HIST, f), 'utf8')));
}
...
console.log('\n=== (a) TRAJECTORY h12->h15 (collapse then surge) ===');
...
console.log('\n=== (b) MINTING h13-h15 (post-2,002-record) ===');
const m = ['2026-08-14-12', '2026-08-14-13', '2026-08-14-14', '2026-08-14-15'].map(h => batch[h] || 0);
console.log(`minted h12/h13/h14/h15 = ${m.join('/')}`);
if (m.slice(1).some(v => v >= 500)) {
  console.log('=> FACTORY STILL RE-ARMING after the record -> another surge wave likely');
} else {
  console.log('=> factory decelerated after the 2,002 record (deploy-then-re-arm, or exhausted)');
}

console.log('\n=== (c) WATCHED ACTORS + TOP5 FARMS (h12-h15) ===');
const watches = ['LiamBruhin', 'ugmoddev', 'jvhoang', 'elad-cmd', 'loganfoxdale', 'williamsjacob', 'sweeneyrachel', 'trujillojoshua', 'thompsoncarrie', 'taylorlisa', 'matkcrais', 'winson', 'itaalo67'];
...
console.log('\n=== (e) NAMING TEMPLATE (h13-h15 demoted actors, first 40 each) ===');
const patA = /^[a-z]+[a-z0-9]+\d{2,}$/;   // firstname+lastname+digits
const patG = /^[a-z]{4,12}$/;             // lowercase gibberish word
const patH = /^[a-z]+-\d{5,}$/;           // hyphen+digits (winson-00178005)
const patN = /^\d{5,}$/;                  // bare numeric
for (const h of ['2026-08-14-13', '2026-08-14-14', '2026-08-14-15']) {
...
console.log('\n=== VERDICT ===');
const h13 = series.find(s => s.hour === '2026-08-14-13');
const h14 = series.find(s => s.hour === '2026-08-14-14');
const h15 = series.find(s => s.hour === '2026-08-14-15');
const landed = [h13, h14, h15].some(s => s && s.push_spam_pct >= 50);
console.log(landed
  ? 'LEAD-TIME RULE: PASSED 3RD LIVE TEST (strongest signal yet: record 2,002 mint -> breach within 2h)'
  : 'LEAD-TIME RULE: MISSED — record mint did NOT produce a surge');
