// run38_analysis.cjs — h4-h6: does the re-breach hold? does the factory keep minting?
// Questions from run 37's plan:
//  (a) h3 batch (708) -> h4/h5 hold? lead-time 1h rule live test #2
//  (b) factory keeps minting or sprint-then-trickle (Aug 14 pattern)?
//  (c) ugmoddev: 169/hr = real fatigue or re-ramp? does noti-api-server return (dual-repo)?
//  (d) elad-cmd full-time streak - breaks when?
//  (e) zerotraceh1 return timing; ThatOneFrench one-hit or veteran?
//  (f) srjordan6 second night - one-hit again or returning-veteran pattern?
const fs = require('fs');
...
console.log('\n=== (a) SPAM TRAJECTORY h1 -> h6 Aug16 (re-breach hold? lead-time test #2) ===');
let prev = null;
...
console.log('\n=== (b) MINTING h13 Aug15 -> h6 Aug16 (sprint or trickle?) — exact-hour lookups ===');
const mintKeys = ['2026-08-15-13','2026-08-15-14','2026-08-15-15','2026-08-15-16','2026-08-15-17','2026-08-15-18','2026-08-15-19','2026-08-15-20','2026-08-15-21','2026-08-15-22','2026-08-15-23','2026-08-16-0','2026-08-16-1','2026-08-16-2','2026-08-16-3','2026-08-16-4','2026-08-16-5','2026-08-16-6'];
...
// hourly actor rates h1 -> h6 from bot_watch
const hourly = {};
...
console.log('\n=== (c/d/e/f) VETERAN TRACE h1 -> h6 ===');
const vets = ['ugmoddev', 'elad-cmd', 'zerotraceh1', 'Janfindl', 'ThatOneFrench', 'srjordan6', 'danialzivehdadr', 'winson-00178005', '2awillican988', 'rite83danil', 'nathydia', 'carroweimer'];
for (const v of vets) {
  const rates = hseq.map(h => `${h.slice(11)}:${hourly[h][v] || 0}`).join('  ');
  console.log(`${v.padEnd(18)} ${rates}`);
}

console.log('\n=== (c) ugmoddev DUAL-REPO trace h1 -> h6 ===');
const repoHour = {};
for (const s of series) {
  if (cmp(s.hour, '2026-08-16-1') < 0) continue;
  const parts = [];
...
console.log('\n=== (d) elad-cmd FULL TRACE (full-time streak check) ===');
const hourly4 = {};
for (const s of series) {
  hourly4[s.hour] = 0;
  for (const f of (s.bot_watch || [])) {
...
console.log(el.slice(-60).map(h => `${h.slice(5)}:${hourly4[h]}`).join('  '));

console.log('\n=== (e) Janfindl FULL TRACE (resurrection timeline) ===');
const hourly5 = {};
for (const s of series) {
  hourly5[s.hour] = 0;
  for (const f of (s.bot_watch || [])) {
...
console.log(jf.slice(-48).map(h => `${h.slice(5)}:${hourly5[h]}`).join('  '));

console.log('\n=== (e) ThatOneFrench FULL TRACE (one-hit or veteran?) ===');
const hourly7 = {};
for (const s of series) {
  hourly7[s.hour] = 0;
  for (const f of (s.bot_watch || [])) {
...
console.log(tof.slice(-24).map(h => `${h.slice(5)}:${hourly7[h]}`).join('  '));

console.log('\n=== (f) srjordan6 FULL TRACE (returning-veteran pattern?) ===');
const hourly8 = {};
for (const s of series) {
  hourly8[s.hour] = 0;
  for (const f of (s.bot_watch || [])) {
...
console.log('\n=== (e) zerotraceh1 FULL TRACE (return timing) ===');
const hourly3 = {};
for (const s of series) {
  hourly3[s.hour] = 0;
  for (const f of (s.bot_watch || [])) {
...
console.log('\n=== (e) NAMING TEMPLATES h2-h6 (top-15 farm actors) ===');
for (const s of series) {
  if (cmp(s.hour, '2026-08-16-2') < 0) continue;
...
console.log('\n=== FARMS h4-h6 (bot_watch top-5 per hour) ===');
for (const s of series) {
  if (cmp(s.hour, '2026-08-16-4') < 0) continue;
  const bw = (s.bot_watch || []).slice(0, 5).map(f => `${f.repo}(${f.pushes})`).join('  ');
  console.log(`${s.hour}  n=${(s.bot_watch||[]).length}  ${bw}`);
}

// botnet_watch (persistent 13h+ farms) h4-h6
console.log('\n=== BOTNET WATCH h4-h6 (persistent farms) ===');
for (const s of series) {
  if (cmp(s.hour, '2026-08-16-4') < 0) continue;
